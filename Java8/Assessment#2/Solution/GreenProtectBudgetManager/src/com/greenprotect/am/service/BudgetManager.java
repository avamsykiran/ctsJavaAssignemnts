...
public class BudgetManager {
	...
	public Map<String, Double> getFieldWiseBudget(String field, Aggregate aggregate) throws IllegalFieldNameException {
		...
		Stream<ActivityDetailsVO> activityStream = activites.stream();
		...
		case "location":
			...
			case SUM:
				result = activityStream
						.collect(Collectors.toMap(a -> a.getLocation(), a -> a.getBudget(), (b1, b2) -> b1 + b2));
				break;
			case MIN:
				result = activityStream.collect(
						Collectors.toMap(a -> a.getLocation(), a -> a.getBudget(), (b1, b2) -> b1 < b2 ? b1 : b2));
				break;
			case MAX:
				result = activityStream.collect(
						Collectors.toMap(a -> a.getLocation(), a -> a.getBudget(), (b1, b2) -> b1 > b2 ? b1 : b2));
			...
			break;
		case "precedingOfficer":
			switch (aggregate) {
			case SUM:
				result = activityStream
						.collect(Collectors.toMap(a -> a.getPrecedingOfficer(), a -> a.getBudget(), (b1, b2) -> b1 + b2));
				break;
			case MIN:
				result = activityStream.collect(
						Collectors.toMap(a -> a.getPrecedingOfficer(), a -> a.getBudget(), (b1, b2) -> b1 < b2 ? b1 : b2));
				break;
			case MAX:
				result = activityStream.collect(
						Collectors.toMap(a -> a.getPrecedingOfficer(), a -> a.getBudget(), (b1, b2) -> b1 > b2 ? b1 : b2));
		...
		case "title":
			...
			case SUM:
				result = activityStream
						.collect(Collectors.toMap(a -> a.getTitle(), a -> a.getBudget(), (b1, b2) -> b1 + b2));
				break;
			case MIN:
				result = activityStream.collect(
						Collectors.toMap(a -> a.getTitle(), a -> a.getBudget(), (b1, b2) -> b1 < b2 ? b1 : b2));
				break;
			case MAX:
				result = activityStream.collect(
						Collectors.toMap(a -> a.getTitle(), a -> a.getBudget(), (b1, b2) -> b1 > b2 ? b1 : b2));
			...
			break;			
		default:
			throw new IllegalFieldNameException("No Such Field");
		}
	...
	public ActivityDetailsVO getCostliestActivity() throws IllegalFieldNameException {
		...
		activity=activites.stream().reduce((a1,a2)->a1.getBudget()>a2.getBudget()?a1:a2).orElse(null);
	...
	public Map<String, Double> getMonthWiseBudget() {
		...
		DateTimeFormatter formater = DateTimeFormatter.ofPattern("MMM-yyyy");
		
		result= activites.stream().collect(Collectors.toMap(
				a -> a.getActivityDate().format(formater)
				, a -> a.getBudget(), (b1,b2)->b1+b2));
